...
// Common types are highlighted differently, terminated
// here with | to tell the difference
char* C_HL_keywords[] = {
...
void editorUpdateSyntax(erow* row) {
    ...
    // Consider the beginning of a line as a separator
    int prev_sep = 1;
    // We aren't initialy in a string or a multiline comment
    int in_string = 0;
    ...
    while (i < row->rsize) {
        ...
        if (prev_sep) {
            ...
            for (j = 0; keywords[j]; j++) {
                ...
                // If it's a common type, subtract the indicator
                if (kw2) {
                    klen--;
                }
...
void editorFindCallback(char* query, int key) {
    ...
    for (i = 0; i < E.numrows; i++) {
        ...
        // Check if the query is a substring of row->render
...
void editorDrawRows(struct abuf *ab) {
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        if (filerow >= E.numrows) {
            ...
            for (j = 0; j < len; j++) {
                if (iscntrl(c[j])) {
                    // Translate control characters into printable ones
                    char sym = (c[j] < 26) ? '@' + c[j] : '?';
